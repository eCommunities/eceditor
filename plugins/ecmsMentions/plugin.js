...
CKEDITOR_mentions.prototype.timeout_callback = function (args) {
  ...
  // Only query at 3+ chars (inc @), and quit if space (' ') typed
  if (str.length < CKEDITOR_mentions.start_observe_count || str.includes(' ')) {
  ...
  var authDataAttr  = editor.config.ecms_mentionsAuthAttribute || 'mentions-auth';
  var mentionsUrl   = editor.config.ecms_mentionsEndpoint || '/api/v1.0/ecugm/user/';
  var profileUrlTpl = editor.config.ecms_mentionsProfileUrl || '/ecms/ECUGM/edit_user.php?user_id={uid}';
  var auth_sig      = $('textarea#' + element_id).data(authDataAttr);

  $.get(mentionsUrl, {terms: term, auth_signature: auth_sig }, function(response) {
    var ckel = $('#' + element_id);
    var par = ckel.parent();
    ...
    if (response.data) {
		var menu = '<div class="col-xs-12 mention-suggestions">';
    	$.each(response.data,function(i, suggestion) {
    		var condensed_name = suggestion.firstname + suggestion.lastname;
    		if (suggestion.email && !condensed_name) {
    			menu = menu + '<a class="mention-users" data-uid="' + suggestion.userid + '" data-realname="' + suggestion.email + '">' + suggestion.email + '</a>';
    		} else if (condensed_name && !suggestion.email) {
    			menu = menu + '<a class="mention-users" data-uid="' + suggestion.userid + '" data-realname="' + condensed_name + '">' + suggestion.firstname + ' ' + suggestion.lastname + '</a>';
    		} else {
    			menu = menu + '<a class="mention-users" data-uid="' + suggestion.userid + '" data-realname="' + condensed_name + '">' + suggestion.firstname + ' ' + suggestion.lastname + ' (' + suggestion.email + ')</a>';
    		}
    	});
    	menu = menu + '</div>';
    	$(menu).insertAfter(par);
  	}
    ...
    $('.mention-users').click(function(e) {
      ...
      template.innerHTML = '<ecms_mention_user contenteditable="false" data-uid="' + $(this).data('uid') + '"><a href="' + profileUrlTpl.replace('{uid}', encodeURIComponent($(this).data('uid'))) + '">@' + $(this).data('realname') + '</a></ecms_mention_user>';
      var link = template.content.firstChild;
...
///////////////////////////////////////////////////////////////
(function($){
  CKEDITOR.plugins.add('ecmsMentions', {
    icons: '',
...
( function() {
	...
	CKEDITOR.plugins.add('mentions', {
		...
		init : function(editor) {
	         var mentionsCommand = editor.addCommand('mentions', {
