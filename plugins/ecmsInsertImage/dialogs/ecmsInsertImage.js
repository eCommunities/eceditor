...
(function()
{
	var imageDialog = function( editor, dialogType )
	{
		...
		var onSizeChange = function()
		{
			...
			// Update the ECMS Media URL
			ecmsUpdateUrl( dialog );

			updatePreview( dialog );
		};

		var ecmsUpdateUrl = function( dialog ) {
			if (dialog.lockRatio) {
				var newSid = 'sid='+ (dialog.getValueOf('info','txtWidth') || '1')+'x'+(dialog.getValueOf('info','txtHeight') || '1');
			} else {
				if (dialog.getValueOf('info','txtWidth') > dialog.getValueOf('info','txtHeight')) {
					var newSid = 'sid='+(dialog.getValueOf('info','txtWidth') || '1')+'x'+(dialog.getValueOf('info','txtWidth') || '1');
				} else {
					var newSid = 'sid='+(dialog.getValueOf('info','txtHeight') || '1')+'x'+(dialog.getValueOf('info','txtHeight') || '1');
				}
			}
			window.supressUrlChange = true;
			var newUrl = dialog.getValueOf('info','txtUrl').replace(/sid=\d{1,}x\d{1,}/, newSid);
		...
		// field (cssStyle) higher priority to avoid overwriting styles contribute
		// by other fields.
		function commitContent()
		{
			var args = arguments;
			var inlineStyleField = this.getContentElement( 'advanced', 'cssStyle' );
			inlineStyleField && inlineStyleField.commit.apply( inlineStyleField, args );

			this.foreach( function( widget )
			{
				if ( widget.commit &&  widget.id != 'cssStyle' )
					...
					[
						{
							...
							html : 'Select or update the image details, to select a new image or replace the existing one, use the Browse... button.'
						},
						{
							type : 'vbox',
							padding : 0,
									...
									[
										{
											...
											onChange : function()
											{
												// Changes in dimensions will trigger this due to URL rewriting taking place in onSizeChange, check for document.supressUrlChange.
												if (window.supressUrlChange == true) {
													window.supressUrlChange = false;
													return;
												}
										...
										},
										{
											...
											[
												...
												},
												{
													id : 'ratioLock',
													type : 'html',
													style : 'margin-top:15px; width:40px; height:20px;',
													onLoad : function()
													{
										...
										},
										{
											...
											[
												...
												},
												{
													...
													widths : [ '35%','65%' ],
													style : 'width:90px',
													label : 'Align <span style="font-size:10px;">(Float Image)</span>',
													'default' : '',
													items :
													[
	...
	CKEDITOR.dialog.add( 'ecmsInsertImageDialog', function( editor )
		{
			return imageDialog( editor, 'ecmsInsertImage' );
		});
